import {
  ...
  PAGE_TRANSITION_KIND_MAP, USER_COMMAND_KEY_MAP, SUMMARY_TOGGLE_KEY,
...
import { DelayBuffer } from '../core/audio-renderer/delay-buffer.js';
...
const MODE_BUTTON_LIST = [
  { mode: MODE_MAP.listening, label: '1.1 청취' },
  { mode: MODE_MAP.delayed, label: '1.2 지연' },
  { mode: MODE_MAP.realtimeScu, label: '1.3 실시간[1]' },
  { mode: MODE_MAP.realtimeMix, label: '1.4 실시간[2]' },
];
const modeButtonMap = new Map();
...
function startNarrationPlayer({ stream }) {
  ...
  if (policy?.isMixed) {
    ...
    console.log('[mix] 1.4 믹싱 모드 — 회의음 우측 덕킹 / 나레이션 좌측');
    return;
  }
...
function startDelayBufferIfNeeded({ fromTimeMs = null } = {}) {
  { $('stBuffer').textContent = '실회의 — 지연 없음'; return; }
  const policy = getModePolicy({ mode: getCurrentMode() });
  ...
  if (!policy || policy.delayMs <= 0) {
    markDebug({
      stage: DEBUG_STAGE_MAP.session, verdict: DEBUG_VERDICT_MAP.info,
      reason: '실시간 모드 — 지연 버퍼 없음',
      detail: `모드=${getCurrentMode()} 배속 ${policy?.rateMin}~${policy?.rateMax} · 나레이션은 생성 즉시 삽입`,
    });
    return;
  }
  delayBuffer = new DelayBuffer({
    getLiveVideo: () => captureSource.liveVideo,
    targetDelayMs: policy.delayMs,
    rateMin: policy.rateMin,
    rateMax: policy.rateMax,
    onBufferChange: handleDelayBufferChange,
  });
  const delayedVideo = delayBuffer.start({ sourceUrl: captureSource.objectUrl, fromTimeMs });
  captureSource.detachDirectAudio();
  document.body.appendChild(delayedVideo);
...
function describeModeOutput({ mode }) {
  ...
  if (!policy) return '';
  if (policy.isMixed) return '서라운드 믹싱 — 좌 나레이션 / 우 회의음 (정지 없음)';
  const { rateMin, rateMax } = getCatchupRateRange({ mode });
  if (rateMax <= 1.0) return 'SCU 삽입 — 배속 회수 없음 (원음 1.0배)';
  return `SCU 삽입 — ${rateMin}~${rateMax}배로 회수`;
}

function buildModeButtons() {
  const fragment = document.createDocumentFragment();
  for (const entry of MODE_BUTTON_LIST) {
    ...
    out.className = 'mode-out';
    out.textContent = describeModeOutput({ mode: entry.mode });
    button.append(name, out);
    button.addEventListener('click', () => handleClickModeButton({ mode: entry.mode }));
    fragment.append(button);
    modeButtonMap.set(entry.mode, button);
  }
  $('modeBar').replaceChildren(fragment);
  $('modeNote').textContent = IS_DELAY_BUFFER_ENABLED
    ? '실험 중에는 실시간 모드끼리만 전환할 수 있습니다 (청취·지연은 시작 시 고정).'
      + ' 청취·지연 모드는 시작 직후 지연이 쌓일 때까지(각 1분·10초) 준비 화면이 표시됩니다.'
    : '지연 기능은 지금 꺼져 있습니다 — 네 모드 모두 라이브 위에서 돌고, 차이는 소리 출력 방식뿐입니다.'
      + ' 실험 중에도 아무 때나 눌러 바꿀 수 있습니다.';
  renderModeButtons();
}

function renderModeButtons() {
  for (const [mode, button] of modeButtonMap) {
    button.classList.toggle('on', mode === currentMode);

    if (!getModePolicy({ mode })?.isMixed) { button.disabled = true; continue; }
    button.disabled = captureSource.isActive
      && !canSwitchMode({ fromMode: currentMode, toMode: mode });
  }
  {
    $('modeNote').textContent = '실회의에서는 1.4 실시간 믹싱만 지원합니다'
      + ' — 나머지 모드의 지연 버퍼는 파일 재생을 전제로 만들어져 라이브 스트림에서 성립하지 않습니다.';
  }
}
...
function handleClickModeButton({ mode }) {
  ...
  if (captureSource.isActive && !canSwitchMode({ fromMode: currentMode, toMode: mode })) {
    showNotice('지연·청취 모드로 전환한 뒤에는 되돌릴 수 없습니다'
      + ' (실시간 → 지연·청취 방향만 가능 · 이미 들은 구간을 건너뛰지 않기 위해서입니다)');
    return;
  }
...
function applyDelayBufferForMode() {
  ...
  const policy = getModePolicy({ mode: getCurrentMode() });
  if (!policy || policy.delayMs <= 0 || delayBuffer) return false;
...
async function pollSpeakerIdentity() {
  ...
  // VLM이 알아낸 이름은 RoiStore가 모른다 — 전사 화자 힌트로 쓰려면 여기서 받아 둬야 한다
...
async function handleUserCommand({ command }) {
  ...
  if (!captureSource.isActive) {
    showNotice('실험을 시작해야 커맨드를 쓸 수 있습니다');
    return;
  }
  const buffer = await captureFrameJpeg();
  if (!buffer) return;
  ...
  const result = await window.seeon.sendPipelineCommand({ ts, buffer, command, wordBudget })
    .catch(() => null);
...
function isNarrationEchoWindow() {
  const now = performance.now();
  if (getNarrationPlayer()?.getScuState?.().isInserting) {
...
setInterval(() => {
  if (!captureSource.isActive) return;
  // 스피커로 나간 우리 나레이션이 참가자 마이크를 타고 회의로 되돌아온다. 그것을 발화로 세면
  // 무음 구간에도 발화자 폴링이 멈추지 않는다
...
async function attemptStartExperiment() {
  ...
  try {
    $('captionBar').textContent = '캡션이 여기에 실시간으로 표시됩니다.';
