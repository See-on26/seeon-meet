import { SCU_CATCHUP_MIN_RATE, SCU_CATCHUP_MAX_RATE, SCU_CATCHUP_RATE_PER_SEC } from '../constants.js';
...
export class CaptionNarrator {
  constructor({ getLiveVideo, onStatus = null, getRateRange = null }) {
    this.getLiveVideo = getLiveVideo;
    this.onStatus = onStatus || (() => {});
    this.getRateRange = getRateRange
      || (() => ({ rateMin: SCU_CATCHUP_MIN_RATE, rateMax: SCU_CATCHUP_MAX_RATE }));
  ...
  _startCatchup(pausedSec) {
    ...
    const rate = computeCatchupRate({
      pausedSec, minRate: rateMin, maxRate: rateMax, ratePerSec: SCU_CATCHUP_RATE_PER_SEC,
    });
