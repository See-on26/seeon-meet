...
export class RoiStore {
  ...
  // 공유화면이 사라진 뒤 부재가 확정되기를 기다리는 동안에는 좌표를 내주지 않는다.
  // 히스테리시스는 "상태를 아직 허물지 않기" 위한 것이지 "낡은 좌표를 계속 믿기" 위한 것이 아니다.
  // 실측(2026-08-09): 공유가 끝난 자리에 Meet이 카메라 타일을 채웠고, 우리는 1.5초 동안 그것을
  // 슬라이드로 잘라 "hyun이라는 문구가 적힌 슬라이드"를 만들었다
  ...
  getParticipantRectNorm() {
    this._refreshStaleState();
    // 본인 타일은 뺀다 — 발화자 후보가 될 수 없는데(self_only는 고지하지 않는다) 크롭에 들어가면
    // VLM에게 강조 타일을 고르라며 답이 아닌 선택지를 하나 더 주는 셈이다
