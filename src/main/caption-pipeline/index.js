...
function registerPipeline({ getViewerView }) {
  ...
  async function sendCaption({ ts, text, source, slideLabel = null, isDedupeExempt = false,
    ...
    const duplicate = isDedupeExempt
      ...
      : narrationHistory.findDuplicate({ text, source });
    ...
    narrationHistory.remember({ text, source, ts });
    let wavData = null;
    try {
  ...
  ipcMain.handle('pipeline-session-start', async (_event, { mode = '' } = {}) => {
    ...
    console.log(`[pipeline] ■ 실험 세션 시작 (모드=${mode}, ${sessionId}) — 중복 이력·화면 레지스트리 초기화`);
    markDebug({
  ...
  ipcMain.handle('pipeline-session-end', async () => {
    ...
    console.log(`[pipeline] ■ 실험 세션 종료 (${sessionId}) — 발화 ${lastSession.asrEntryList.length}건 · 슬라이드 마크 ${lastSession.slideMarkList.length}건`);
    return {
      isOk: true,
      sessionId,
  ...
  ipcMain.handle('pipeline-frame', async (_event, { ts, buffer, slideLabel, wordBudget, isRevisit }) => {
    ...
    try {
      ...
      // 이미지 임베딩은 같은 슬라이드 0.100 / 다른 슬라이드 0.102로 붙어 있어 동일성을 못 가른다.
      // 주제 텍스트는 0.824 / 0.154로 갈려 여기서 최종 판정한다 (실측 2026-08-09)
      ...
      if (!isRevisit && topicVerdict.kind === SLIDE_TOPIC_VERDICT_MAP.same) {
        await appendRecord({
          kind: RECORD_KIND_MAP.pageTransition, ts, slideLabel, screenType: result.screenType,
      ...
      const isRevisitFinal = isRevisit || topicVerdict.kind === SLIDE_TOPIC_VERDICT_MAP.revisit;
      slideTopicRegistry.remember({
        topic: result.pageTopic, description: result.description, label: slideLabel,
      });
      markDebug({
        stage: DEBUG_STAGE_MAP.slide, verdict: DEBUG_VERDICT_MAP.info, ts,
        narrationType: NARRATION_TYPE_MAP.pageTransition,
        reason: `주제 판정 ${topicVerdict.kind}`, detail: topicDetail,
      });

      if (isRevisitFinal) {
        const revisitNarration = `이전 슬라이드 ${slideMajor || slideLabel}: ${result.pageTopic || result.narration}`;
        await appendRecord({
          kind: RECORD_KIND_MAP.pageTransition, ts, slideLabel, screenType: result.screenType,
      ...
      // 말이 맺히지 않은 묘사는 에이전트가 빈 문자열로 돌려준다 — 어색하게 읽어 주느니 침묵한다
      if (!result.narration) {
        await appendRecord({
          kind: RECORD_KIND_MAP.pageTransition, ts, slideLabel, screenType: result.screenType,
      ...
      const narration = result.screenType === 'ppt' && slideMajor
        ? `슬라이드 ${slideMajor}: ${result.narration}`
        : result.narration;
      await appendRecord({
        kind: RECORD_KIND_MAP.pageTransition, ts, slideLabel, screenType: result.screenType,
  ...
  ipcMain.handle('pipeline-deixis', async (_event, { ts, buffer, utterance, recentTranscript, beforeTranscript, afterTranscript, hasPointingRegion, pointingOrderHint, slideLabel, wordBudgetMap }) => {
    ...
    try {
      ...
      const { text, isGrounded } = await agent.generate({
      ...
      const sent = await sendCaption({
        ts, text, source: narrationType, slideLabel,
        rationale: {
          utterance,
          subKind: selectedCandidate?.subKind || null,
  ...
  ipcMain.handle('pipeline-command', async (_event, { ts, buffer, command, wordBudget }) => {
    const stage = PIPELINE_STAGE_MAP.vlm;
    const startedAt = Date.now();
    console.log(`[pipeline] ▶ 사용자 커맨드 ${command} ts=${ts}`);
    try {
      const { text, isGrounded } = await agentFieldMap[NARRATION_TYPE_MAP.userCommand].generate({
        jpegBuffer: Buffer.from(buffer), command,
        minWordCount: wordBudget?.minWordCount, maxWordCount: wordBudget?.maxWordCount,
      });
      await appendRecord({
        kind: RECORD_KIND_MAP.deixis, ts, utterance: '', caption: text, isGrounded,
        narrationType: NARRATION_TYPE_MAP.userCommand, subKind: command,
      });
      if (!isGrounded || !text) {
        console.log(`[pipeline] 커맨드 응답 없음 (${Date.now() - startedAt}ms) ${command}`);
        return { isOk: true, isGrounded: false };
      }
      console.log(`[pipeline] 커맨드 응답 (${Date.now() - startedAt}ms) ${command} → "${truncateForLog(text)}"`);
      markDebug({
        stage: DEBUG_STAGE_MAP.vlm, verdict: DEBUG_VERDICT_MAP.pass, ts,
        narrationType: NARRATION_TYPE_MAP.userCommand,
        reason: `커맨드 ${command} 응답`, detail: `"${text}"`,
      });

      await sendCaption({
        ts, text, source: NARRATION_TYPE_MAP.userCommand, isDedupeExempt: true,
        rationale: { subKind: command, summary: '사용자가 단축키로 직접 요청한 답변' },
      });
  ...
  ipcMain.handle('pipeline-gap', async (_event, { ts, narrationText, narrationType, subKind,
    rationale, screenSummary, beforeTranscript, gapList }) => {
    const startedAt = Date.now();
    const result = await gapAgent.infer({
      narrationText, narrationType, subKind, rationale, screenSummary, beforeTranscript,
      gapList: gapList || [],
    });
